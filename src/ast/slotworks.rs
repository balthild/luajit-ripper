...
pub fn eliminate_temporary(root: &NodeRef, options: Options) -> Result<()> {
    let invalidated = Invalidated::default();

    eliminate_multres(root, &invalidated)?;

    let mut slots = collect_slots(root, options.identify_slots, options.unwarped);
    sort_slots(&mut slots);
    eliminate_collected(root, slots, options, &invalidated)?;

    if !options.unwarped {
        cleanup_invalid_nodes(root, &invalidated);
    }
...
pub fn simplify_ast(root: &NodeRef, callback: &mut dyn FnMut(&NodeRef)) {
    traverse::traverse(&mut SimplifyVisitor::new(callback), root);
}
...
struct Invalidated(Rc<RefCell<HashSet<usize>>>);
...
impl Invalidated {
    fn mark(&self, node: &NodeRef) {
        self.0.borrow_mut().insert(node_key(node));
    }

    fn is_marked(&self, node: &NodeRef) -> bool {
        self.0.borrow().contains(&node_key(node))
    }
}

/// Drops the statements the pass marked as deleted.
fn cleanup_invalid_nodes(root: &NodeRef, invalidated: &Invalidated) {
    traverse::traverse(&mut TreeCleanup { invalidated }, root);
}

struct TreeCleanup<'a> {
    invalidated: &'a Invalidated,
}

impl Visitor for TreeCleanup<'_> {
    fn visit(&mut self, node: &NodeRef) -> bool {
        ...
        let kept = contents
            ...
            .collect();
        traverse::set_block_contents(node, kept);
...
fn is_slot_identifier(node: &NodeRef) -> Option<u32> {
    match &*node.borrow() {
...
fn identifier_slot(node: &NodeRef) -> Option<u32> {
    match &*node.borrow() {
...
fn identifier_id(node: &NodeRef) -> Option<u32> {
    match &*node.borrow() {
...
fn identifier_possible_ids(node: &NodeRef) -> Vec<u32> {
    match &*node.borrow() {
        Node::Identifier(identifier) => identifier.possible_ids.clone(),
        _ => Vec::new(),
    }
}

fn identifier_name(node: &NodeRef) -> Option<String> {
    match &*node.borrow() {
...
fn identifier_kind(node: &NodeRef) -> Option<IdentifierKind> {
    match &*node.borrow() {
...
fn set_identifier_id(node: &NodeRef, id: u32) {
    if let Node::Identifier(identifier) = &mut *node.borrow_mut() {
        identifier.id = Some(id);
    }
}

fn set_identifier_possible_ids(node: &NodeRef, ids: Vec<u32>) {
    if let Node::Identifier(identifier) = &mut *node.borrow_mut() {
        identifier.possible_ids = ids;
    }
}

fn forget_possible_id(node: &NodeRef, id: u32) {
    if let Node::Identifier(identifier) = &mut *node.borrow_mut() {
        identifier.possible_ids.retain(|candidate| *candidate != id);
    }
}

fn is_list(node: &NodeRef) -> bool {
    matches!(
        &*node.borrow(),
...
fn get_holder(path: &[NodeRef]) -> Option<NodeRef> {
    path.iter()
        .rev()
        .skip(1)
...
fn assignment_source(assignment: &NodeRef) -> Result<NodeRef> {
...
fn replace_in_list(list: &NodeRef, original: &NodeRef, replacement: &NodeRef) -> bool {
    let contents = traverse::list_contents(list);
    match traverse::position(&contents, original) {
        Some(index) => {
            let mut contents = contents;
            contents[index] = replacement.clone();
            traverse::set_list_contents(list, contents);
            true
        }
        None => false,
    }
...
fn replace_node(holder: &NodeRef, original: &NodeRef, replacement: &NodeRef) -> bool {
    if is_list(holder) {
        return replace_in_list(holder, original, replacement);
    }
    traverse::replace_child(holder, original, replacement.clone())
}
...
type KnownSlots = HashMap<u32, HashMap<i64, Info>>;

// -- collecting ------------------------------------------------------------

struct CollectorState {
    known_slots: KnownSlots,
    all_known_slots: KnownSlots,
    /// The state every finished block was left in, by block identity.
    block_slots: HashMap<usize, KnownSlots>,
        ...
        for slot_states in self.known_slots.values() {
            for (id, info) in slot_states {
                if i64::from(info.borrow().slot_id) == *id
                    && !infos.iter().any(|existing| Rc::ptr_eq(existing, info))
                {
...
struct SlotsCollector {
    states: Vec<CollectorState>,
    path: Vec<NodeRef>,
...
impl SlotsCollector {
    fn run(root: &NodeRef, identify: bool, unwarped: bool) -> (Vec<Info>, Vec<Info>) {
    ...
    fn find_slot_assignments(
        ...
        let current = state.block.as_ref().map(node_key);
        ...
        let mut found: Vec<Info> = Vec::new();
        for predecessor in predecessors {
            ...
            if let Some(infos) = self.find_slot_assignments(slot, Some(*predecessor), visited) {
                for info in infos {
                    if !found.iter().any(|existing| Rc::ptr_eq(existing, &info)) {
                        found.push(info);
                    }
                }
            }
    ...
    fn commit_info(&mut self, info: &Info) {
        match info.borrow().references.len() {
            0 => {}
            1 => self.unused.push(info.clone()),
            _ => self.slots.push(info.clone()),
        }
    }

    /// Ends the life of the definition `id` refers to.
    fn commit_slot(&mut self, slot: u32, id: Option<u32>, node: &NodeRef) {
        let Some(info) = self.get_slot(slot, id, true) else {
            return;
        };

        info.borrow_mut().termination = Some(node.clone());
        self.remove_slot(slot, id);
        self.commit_info(&info);
    }

    /// Starts a new definition of `slot`.
    fn register_slot(&mut self, node: &NodeRef, slot: &NodeRef, assignment: &NodeRef) {
        ...
        let info = Rc::new(RefCell::new(SlotInfo {
            slot: slot_number,
    ...
    fn register_all_slots(&mut self, node: &NodeRef, slots: &[NodeRef]) {
        for slot in slots {
    ...
    fn register_slot_reference(&mut self, info: &Info, node: &NodeRef, update_id: bool) {
        ...
        if id.is_none() && !possible_ids.contains(&slot_id) {
            if update_id {
                ...
                set_identifier_possible_ids(node, possible_ids);
            }
        }
    ...
    fn leave_block(&mut self, node: &NodeRef) {
        let targets = match traverse::block_warp(node) {
            Some(warp) => {
                let borrowed = warp.borrow();
                traverse::block_targets(&borrowed)
            }
            None => Vec::new(),
        };
...
impl Visitor for SlotsCollector {
    fn visit(&mut self, node: &NodeRef) -> bool {
        ...
        enum Action {
            None,
            Enter,
            Assignment(NodeRef, Vec<NodeRef>),
            Identifier,
            Block,
        }
        ...
        match action {
            ...
            Action::Block => self.state().block = Some(node.clone()),
        }
...
fn collect_slots(root: &NodeRef, identify: bool, unwarped: bool) -> Vec<Info> {
    SlotsCollector::run(root, identify, unwarped).0
}
...
struct RefsProcessData {
    slots: Vec<Info>,
    simple: Vec<SimpleReference>,
    massive: Vec<(NodeRef, Info, NodeRef, NodeRef)>,
    tables: Vec<(Info, SlotReference)>,
    iterators: Vec<(Info, NodeRef, NodeRef)>,
    unsafe_slots: Vec<Info>,
}

impl RefsProcessData {
    fn new(slots: Vec<Info>) -> Self {
        RefsProcessData {
            slots,
            simple: Vec::new(),
            massive: Vec::new(),
...
fn eliminate_collected(
    ...
    eliminate_simple_cases(&data.simple, invalidated)?;
    recheck_unsafe_cases(root, &data.unsafe_slots, options, invalidated)?;
    eliminate_into_table_constructors(&data.tables, invalidated)?;
    eliminate_mass_assignments(&data.massive, invalidated);
    eliminate_iterators(&data.iterators, invalidated)?;

    Ok(())
}

/// Decides what to do with every collected register.
fn fill_refs(data: &mut RefsProcessData, ignore_ambiguous: bool, safe_mode: bool) -> Result<()> {
    for info in data.slots.clone() {
        let assignment = info.borrow().assignment.clone();
        ...
        if destination_count > 1 {
            fill_massive_refs(&info, data, ignore_ambiguous, safe_mode)?;
        } else {
            fill_simple_refs(&info, data, ignore_ambiguous, safe_mode)?;
        }
    }
...
fn fill_simple_refs(
    ...
    let mut holders: Vec<NodeRef> = Vec::new();
    let mut new_simple: Vec<SimpleReference> = Vec::new();
    ...
    let references = info.borrow().references.clone();
        ...
        let is_destination = statement.is_some_and(|statement| {
            matches!(&*statement.borrow(), Node::Assignment(inner)
                if traverse::list_contents(&inner.destinations)
                    .first()
                    .is_some_and(|destination| Rc::ptr_eq(destination, &holder)))
        });

        if source_is_table && is_element && is_destination && all_ctor_refs {
            data.tables.push((info.clone(), reference.clone()));
        } else {
    ...
    if count == 1 || (!safe_mode && count == 2) {
        ...
        data.unsafe_slots.push(info.clone());
    }

    Ok(())
}

fn fill_massive_refs(
    info: &Info,
    data: &mut RefsProcessData,
    ignore_ambiguous: bool,
    safe_mode: bool,
    ...
    {
        return fill_simple_refs(info, data, ignore_ambiguous, safe_mode);
    }
    ...
    let Some(first_reference) = info.borrow().references.get(1).cloned() else {
        return Ok(());
    };
    ...
    if safe_mode {
        ...
        loop {
            ...
            let Some(last) = info.borrow().references.last().cloned() else {
                break;
            };
    ...
    match &*holder.borrow() {
        Node::Assignment(inner) => {
            let Some(destination) = traverse::list_contents(&inner.destinations)
                .first()
                .cloned()
            else {
                return Ok(());
            };
            let origin = info.borrow().references[0].identifier.clone();
            ...
            data.massive.push((origin, info.clone(), base, destination));
        }
        Node::IteratorWarp(_) => {
            data.iterators
                .push((info.clone(), source.clone(), holder.clone()));
        }
        _ => {}
    }
...
fn eliminate_simple_cases(simple: &[SimpleReference], invalidated: &Invalidated) -> Result<()> {
    for entry in simple {
        let info = &entry.info;
        ...
        let destination = reference.identifier.clone();
        ...
        if matches!(&*source.borrow(), Node::Primitive(_) | Node::Constant(_)) {
            ...
            if matches!(&*holder.borrow(), Node::FunctionCall(call)
                if Rc::ptr_eq(&call.function, &destination))
            {
                continue;
            }
            if matches!(&*holder.borrow(), Node::TableElement(element)
                if Rc::ptr_eq(&element.table, &destination) && !is_string)
            {
                continue;
            }
        }
        ...
        if !replace_node(&holder, &destination, &source) {
...
fn is_index_key(entry: &SimpleReference) -> bool {
    ...
    matches!(&*holder.borrow(), Node::TableElement(element)
        if Rc::ptr_eq(&element.key, &entry.reference.identifier))
}

/// Folds a register that only assigns into a table constructor into it.
fn eliminate_into_table_constructors(
    tables: &[(Info, SlotReference)],
    invalidated: &Invalidated,
) -> Result<()> {
    for (info, reference) in tables {
        ...
        let element = reference.path[reference.path.len() - 2].clone();
        let base = reference.path[reference.path.len() - 4].clone();
        ...
        let key = match &*element.borrow() {
            Node::TableElement(inner) => inner.key.clone(),
            _ => continue,
        };
        let value = assignment_source(&base)?;

        if insert_table_record(&constructor, &key, &value, false) {
...
fn eliminate_mass_assignments(
    massive: &[(NodeRef, Info, NodeRef, NodeRef)],
    invalidated: &Invalidated,
) {
    for (identifier, info, base_assignment, destination) in massive {
        ...
        let destinations = match &*assignment.borrow() {
            Node::Assignment(inner) => inner.destinations.clone(),
            _ => continue,
        };

        if replace_in_list(&destinations, identifier, destination) {
...
fn eliminate_iterators(
    iterators: &[(Info, NodeRef, NodeRef)],
    invalidated: &Invalidated,
) -> Result<()> {
    ...
    for (info, source, warp) in iterators {
        ...
        let (controls, destinations) = {
            ...
            (inner.controls.clone(), destinations)
        };
        ...
        if let Node::IteratorWarp(inner) = &mut *warp.borrow_mut() {
            inner.controls = expressions(replacement);
        }

        processed.push(warp.clone());
        invalidated.mark(&assignment);
    }
...
fn recheck_unsafe_cases(
    ...
    let mut blocks: Vec<NodeRef> = Vec::new();
    ...
    let mut callback = |node: &NodeRef| {
        if !options.unwarped && !matches!(&*node.borrow(), Node::Block(_)) {
            return;
        }

        cleanup_invalid_nodes(node, &invalidated);

        let mut new_slots = collect_slots(node, false, options.unwarped);
        ...
        .is_ok()
        {
            let _ = eliminate_simple_cases(&data.simple, &invalidated);
        }
    };

    if options.unwarped {
        simplify_ast(root, &mut callback);
    } else {
        for block in blocks {
            simplify_ast(&block, &mut callback);
        }
    }
...
struct SimplifyVisitor<'a> {
    callback: &'a mut dyn FnMut(&NodeRef),
    dirty: bool,
    root: Option<NodeRef>,
}

impl<'a> SimplifyVisitor<'a> {
    fn new(callback: &'a mut dyn FnMut(&NodeRef)) -> Self {
        SimplifyVisitor {
            callback,
            dirty: false,
            root: None,
    ...
    fn flush(&mut self, node: &NodeRef) {
        if self.dirty {
            (self.callback)(node);
            self.dirty = false;
        }
    ...
    fn try_make_method_call(&mut self, node: &NodeRef, function: &NodeRef) {
        let (arguments, is_method) = match &*node.borrow() {
            Node::FunctionCall(inner) => (inner.arguments.clone(), inner.is_method),
            _ => return,
        };
        if is_method {
            return;
        ...
        let Some(first) = argument_contents.first().cloned() else {
            return;
        };
        ...
        traverse::set_list_contents(&arguments, argument_contents);
...
impl Visitor for SimplifyVisitor<'_> {
    fn visit(&mut self, node: &NodeRef) -> bool {
        ...
        let call = match &*node.borrow() {
            Node::FunctionCall(inner) if !inner.is_method => Some(inner.function.clone()),
            _ => None,
        };
...
struct MultresEliminator<'a> {
    last_value: Option<NodeRef>,
    invalidated: &'a Invalidated,
    failure: Option<Error>,
}

impl Visitor for MultresEliminator<'_> {
    fn visit(&mut self, node: &NodeRef) -> bool {
        let list = {
            let borrowed = node.borrow();
            match &*borrowed {
                Node::FunctionCall(inner) => Some(inner.arguments.clone()),
                Node::Return(inner) => Some(inner.returns.clone()),
                _ => None,
            }
        };
            ...
            && let Some(index) = mul_res_index(&list)
        {
            match self.last_value.take() {
                Some(value) => replace_at(&list, index, &value),
    ...
    fn leave(&mut self, node: &NodeRef) {
        ...
        let (destination, source) = {
            ...
            (
                traverse::list_contents(&inner.destinations)
                    .first()
                    .cloned(),
                traverse::list_contents(&inner.expressions).first().cloned(),
            )
        };
        ...
        let expressions = match &*node.borrow() {
            Node::Assignment(inner) => inner.expressions.clone(),
            _ => return,
        };
        ...
        match self.last_value.take() {
            Some(value) => replace_at(&expressions, index, &value),
            None => self.failure = Some(internal("a multiple result without a call")),
        }
    }
}
...
fn mul_res_index(list: &NodeRef) -> Option<usize> {
...
fn replace_at(list: &NodeRef, index: usize, value: &NodeRef) {
    let mut contents = traverse::list_contents(list);
    contents[index] = value.clone();
    traverse::set_list_contents(list, contents);
}

/// Rewrites `MULTRES` back into the call it stands for.
fn eliminate_multres(root: &NodeRef, invalidated: &Invalidated) -> Result<()> {
