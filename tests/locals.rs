...
fn names(root: &NodeRef) -> Vec<String> {
    traverse::walk(root)
        .iter()
...
fn declared_locals(root: &NodeRef) -> Vec<String> {
    traverse::walk(root)
        .iter()
        .filter_map(|node| match &*node.borrow() {
...
fn local_definitions(root: &NodeRef) -> usize {
    traverse::walk(root)
        .iter()
        .filter(|node| {
...
fn build(chunk: &luajit_ripper::bytecode::Chunk) -> NodeRef {
...
fn local_names_are_recovered() {
    ...
    locals::mark_local_definitions(&root);
...
fn a_dump_without_debug_information_keeps_its_slots() {
    ...
    let chunk = luajit_ripper::bytecode::parse(&dump).expect("the dump should parse");
    let root = build(&chunk);
...
fn first_name_of_kind(root: &NodeRef, kind: IdentifierKind) -> Option<String> {
    traverse::walk(root)
        .iter()
...
fn numeric_loop_indices(root: &NodeRef) -> Vec<String> {
    traverse::walk(root)
        .iter()
        .filter_map(|node| match &*node.borrow() {
            Node::NumericLoopWarp(warp) => Some(warp.index.clone()),
            _ => None,
        })
...
fn iterator_loop_variables(root: &NodeRef) -> Vec<String> {
    traverse::walk(root)
        .iter()
        .filter_map(|node| match &*node.borrow() {
